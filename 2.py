text="""Game 1: 9 red, 5 blue, 6 green; 6 red, 13 blue; 2 blue, 7 green, 5 red
...
dummy_text="""Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
def valid_number_of_cubes_in_set(bag_of_cubes):
    flag = True
    for set_of_cubes in bag_of_cubes:
        cube_dict = dict(
            red = 0,
            green = 0,
            blue = 0
        ...
        for cube in cubes:
            ...
            cube_dict[str(color)] += int(count)
        if cube_dict["red"] <= limit_dict["red"] and cube_dict["green"] <= limit_dict["green"] and cube_dict["blue"] <= limit_dict["blue"]:
            pass
        else:
            flag = False
    return flag

def sum_of_power_of_set(bag_of_cubes):
    cube_dict = dict(
            red = 0,
            green = 0,
            blue = 0
    ...
    for set_of_cubes in bag_of_cubes:
        ...
        for cube in cubes:
            count, color = cube.split(" ")
            # check existing value is smaller replace if so
            if cube_dict[str(color)] < int(count):
                cube_dict[str(color)] = int(count)
    return cube_dict["blue"]*cube_dict["green"]*cube_dict['red']

def total_power(lines):
    total_power = 0
    for line in lines:
        game_id, game_detail = line.split(": ")
        bag_of_cubes = game_detail.strip().split(";")
        total_power += sum_of_power_of_set(bag_of_cubes)
    return total_power

def possible_game_id_sum(lines):
    possible_game_int_sum = 0
    for line in lines:
        game_id, game_detail = line.split(": ")
        bag_of_cubes = game_detail.strip().split(";")
        valid_bag = valid_number_of_cubes_in_set(bag_of_cubes)
        if valid_bag:
            possible_game_int_sum += int(game_id.split(" ")[1])
    return possible_game_int_sum

assert 8 == possible_game_id_sum(dummy_text.split("\n"))
print(possible_game_id_sum(text.split("\n")))


assert 2286 == total_power(dummy_text.split('\n'))
print(total_power(text.split("\n")))