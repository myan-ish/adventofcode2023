...
text = """two65eightbkgqcsn91qxkfvg
...
num_in_words = ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"]

num_to_int_map = dict(zip(num_in_words, integers))


def calc_index(string, ele):
    ...
    for line in text.split("\n"):
        if len(line)>0:
            ...
            for element in integers:
                index_calc = line.find(element)
                if not first:
                    first_index = index_calc
                    if first_index!=-1:
                        first = element
                elif index_calc != -1 and first_index>index_calc:
                    first_index = index_calc
                    first = element

            for element in integers:
                index_calc = line[::-1].find(element)
                if not last:
                    last_index = index_calc
                    if last_index!=-1:
                        last = element
                elif index_calc != -1 and last_index>index_calc:
                    last_index = index_calc
                    last = element
            if first or last:
                if first_index != len(line)-last_index-1 and last:
                    total += int(first + last)
                else:
                    total += int(first*2)
    return(total)
...
def second_part(text):
    ...
    for line in text.split("\n"):        
        ...
        first_int_index, last_int_index = (None, None)

        for element in num_in_words:
            index_calc = calc_index(line, element)
            if not first:
                first_index = index_calc
                if first_index!=-1:
                    first = element
        ...
        for element in integers:
            index_calc = calc_index(line, element)
            if not first_int:
                first_int_index = index_calc
                if first_int_index!=-1:
                    first_int = element
            elif index_calc != -1 and first_int_index>index_calc:
                first_int_index = index_calc
                first_int = element

        for element in num_in_words:
            index_calc = [(a.start(), a.end()) for a in list(re.finditer(element, line))]
            if index_calc:
                index_calc = index_calc[-1][0]
                if not last:
                    last_index = index_calc
                    if last_index != -1:
                        last = element
        ...
        for element in integers:
            index_calc = calc_index(line[::-1], element)
            if not last_int:
                last_int_index = index_calc
                if last_int_index!=-1:
                    last_int = element
            elif index_calc != -1 and last_int_index>index_calc:
                last_int_index = index_calc
                last_int = element
        
        # we need check index of first and last value to determine either word or int is first
        if (first_int_index < first_index and first_int_index != -1) or first_index == -1:
            first = first_int
            first_index = first_int_index
        
        print("------------------",line,"-------------------")
        if (last_int_index < len(line)-last_index and last_int_index != -1) or last_index == -1:
            last = last_int
            last_index = last_int_index
        print("first",first, first_index)
        print("last",last, last_index)
        if last and first_index != len(line)-last_index-1:
            first = num_to_int_map[first] if first in num_in_words else first
            last = num_to_int_map[last] if last in num_in_words else last
            total += int(first + last)
